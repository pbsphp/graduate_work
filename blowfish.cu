...
/**
 * Ключ blowfish с массивами P и S.
 */
struct blowfish_key {
   uint p[18];
   uint s[4][256];
...
/**
 * Начальные значения переменных ключа S и P при генерации ключей
 */
 ...
 */
__device__ __host__
static inline uint32_t feistel_function(uint32_t x, struct blowfish_key *key)
{
    uint32_t result;
    result = key->s[0][(x >> 24) & 0xFF];
    result += key->s[1][(x >> 16) & 0xFF];
    result ^= key->s[2][(x >> 8) & 0xFF];
    result += key->s[3][(x >> 0) & 0xFF];
    return result;
}


/**
 * Функция итерации (один раунд в сети фейстеля).
 */
__device__ __host__
static inline void iteration(uint32_t *left, uint32_t *right,
                             int round_num, struct blowfish_key *key)
{
    *left ^= key->p[round_num];
    *right ^= feistel_function(*left, key);
}


/**
 * Шифрование методом blowfish одного 64-битного блока.
 * data - 64-битный блок исходных данных,
 * key - заранее вычисленный ключ blowfish.
 *
 * Шифрование выполняется следующим образом:
 * Входной блок делится на правую и левую части (32-бита каждая),
 * Выполняется 16 раундов Фейстеля, после каждого раунда, за исключением
 * последнего, левая и правая части менятся местами.
 * После этого левая и правая части XOR'ятся с 17 и 18 элементом
 * из массива P ключа и объединяются в один 64-битный блок.
 */
__device__ __host__
static uint64_t blowfish_encrypt_block(
        uint64_t data, struct blowfish_key *key)
{
...
/**
 * Расшифровка методом blowfish одного 64-битного блока.
 * cipher - 64-битный шифротекст,
 * key - заранее вычисленный ключ blowfish.
 *
 * Расшифровка выполняется почти аналогично шифрованию, но в обратном порядке.
 */
__device__
static uint64_t blowfish_decrypt_block(
        uint64_t cipher, struct blowfish_key *key)
{
    uint32_t left = cipher >> 32;
    uint32_t right = cipher & 0xFFFFFFFF;
...
/**
 * Формирует ключ blowfish с массивами P и S из пользовательского ключа.
 * user_key - пользовательский ключ (массив 8-битных данных),
 * len - длина пользовательского ключа (не обязан быть кратным 8).
 * key - структура ключа с массивами P и S для заполнения.
 */
static void key_schedule(
        const uint8_t user_key[], int len, struct blowfish_key *key)
{
    memcpy(key->p, P_PERM, sizeof(uint32_t) * 18);
    memcpy(key->s, S_PERM, sizeof(uint32_t) * 1024);

    for (int idx1 = 0, idx2 = 0; idx1 < 18; ++idx1, idx2 += 4) {
        uint32_t mask = 0;
        for (int i = 0; i < 4; ++i) {
            mask <<= 8;
            mask |= user_key[(idx2 + i) % len];
        }
        key->p[idx1] ^= mask;
    }
    ...
    for (int idx = 0; idx < 18; idx += 2) {
        block = blowfish_encrypt_block(block, key);
        key->p[idx] = (block >> 32) & 0xFFFFFFFF;
        key->p[idx + 1] = block & 0xFFFFFFFF;
    }

    for (int idx = 0; idx < 4; ++idx) {
        for (int idx2 = 0; idx2 < 256; idx2 += 2) {
            block = blowfish_encrypt_block(block, key);
            key->s[idx][idx2] = (block >> 32) & 0xFFFFFFFF;
            key->s[idx][idx2 + 1] = block & 0xFFFFFFFF;
        }
    }
}
...
/**
 * Выполняет шифрование или расшифровку данных.
 * data - данные для шифрования/дешифрования,
 * len - длина данных,
 * key - ключ (структура),
